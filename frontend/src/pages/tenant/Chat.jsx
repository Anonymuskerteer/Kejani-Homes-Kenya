...
export default function Chat() {
  ...
  const handleSendImage = async () => {
    ...
    try {
      ...
      // Send via REST API (multipart/form-data)
  ...
  return (
    <div className="flex flex-col md:flex-row gap-4 p-4 pb-28 lg:pb-4">
      {/* Conversations list */}
      <div className={`w-full md:w-80 flex-shrink-0 border border-border dark:border-dark-border rounded-card bg-background dark:bg-dark-background overflow-hidden flex flex-col ${
        ...
        <div className="p-4 border-b border-border dark:border-dark-border">
          <h2 className="font-semibold text-dark dark:text-light">Messages</h2>
          <p className="text-xs text-muted dark:text-dark-muted mt-1">
            End-to-end encrypted
          </p>
        </div>

        {error && <ErrorState onRetry={fetchConversations} />}
        ...
        {conversations.length > 0 ? (
          ...
          <EmptyState
            ...
            description="Start a conversation with a landlord or property manager"
            icon="chat"
          />
        )}
      </div>
